...
func (t *Test) run(dir, dataDir string) ([]byte, error) {
	testCmd := t.Command
	if len(testCmd) == 0 {
		// No test command given i.e. we assume it's a Python based test in the
		// test/ directory.

		// We skip the build because we just ran "make build" to compile all
		// Go binaries.
	...
	if *docker {
		...
		if *useDockerCache {
			args = append(args, "--use_docker_cache", cacheImage(t.flavor))
		} else {
			// If there is no cache, we have to call 'make build' before each test.
			testArgs = "make build && " + testArgs
		}
		args = append(args, t.flavor, testArgs)
...
// TODO(mberlin): Move this to the top of the file.
func main() {
	flag.Usage = func() {
	...
	// Reshard i.e. re-assign tests to Travis test shards to even out execution time per shard.
	if *reshard > 0 {
		...
		if *pull {
			...
			for _, flavor := range flavors {
				...
				go func(flavor string) {
					...
					log.Printf("Image %v pulled in %v", image, time.Since(pullTime).Truncate(100*time.Millisecond))
				}(flavor)
			}
			wg.Wait()
		}
	...
	if *useDockerCache {
		start := time.Now()
		for _, flavor := range flavors {
			image := cacheImage(flavor)
			log.Printf("Creating Docker cache image for flavor '%s'...", flavor)
			if out, err := exec.Command("docker/test/run.sh", "--create_docker_cache", image, flavor, "make build").CombinedOutput(); err != nil {
				log.Fatalf("Failed to create Docker cache image for flavor '%s': %v\n%s", flavor, err, out)
			}
			log.Printf("Creating Docker cache image took %v", time.Since(start).Truncate(100*time.Millisecond))
		}
	}
		...
		go func() {
			...
			for test := range next {
				...
				for try := 1; ; try++ {
					...
					loggedDuration := duration.Truncate(100 * time.Millisecond)
					...
					if err != nil {
						// This try failed.
						test.logf("FAILED (try %v/%v) in %v: %v", try, tryMax, loggedDuration, err)
						mu.Lock()
					...
					if try == 1 {
						...
						test.logf("PASSED in %v", loggedDuration)
						passed++
					} else {
						// Passed, but not on the first try.
						test.logf("FLAKY (1/%v passed in %v)", try, loggedDuration)
						flaky++
	...
	if *useDockerCache {
		for _, flavor := range flavors {
			...
			if out, err := exec.Command("echo", "docker", "rmi", cacheImage(flavor)).CombinedOutput(); err != nil {
